...
@section('main')
    <div class="w-2/3 p-3 text-slate-300 border border-slate-500 rounded">
        ...
        @include('template.shopping_head', ['step' => 1])
        <h3 class="text-slate-400">訂單明細</h3>
        <form action="{{ route('cart.step02') }}" method="POST">
            @csrf
                ...
                <div class="flex px-2">
                    ...
                    <div class="flex justify-end items-center">
                        <div class="qty" data-id="{{ $product->id }}">
                            ...
                            {{-- 因為是多筆資料所以要用陣列qty[] --}}
                            <input name="qty[]" type="numeric" value="{{ $product->qty }}" min="1"
                                class="add-qty w-14 text-slate-800">
